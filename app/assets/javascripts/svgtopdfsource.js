var SVGtoPDF = function(doc, svg, x, y, options) {
    ...
    function docUsePattern(pattern, stroke) {
      ...
      var ref = doc.ref({
        ...
        Resources: {
          ...
          XObject: (function() {var temp = {}; temp[pattern.group.name] = pattern.group.xobj; return temp;})()
        }
      });
    ...
    var SvgPath = function(d) {
      ...
      var Flags = {A3:true, A4:true, a3:true, a4:true};
       ...
       //   console.log('line974:',value);
          switch (key) {
           ...
           //     console.log('case \'fill\':',value);
                ...
                value = (value || '').split(' ');
                var object = this.resolveUrl(value[0]),
    ...
    var SvgElemLink = function(obj, inherits) {
      SvgElemContainer.call(this, obj, inherits);
      var link = this.attr('href') || this.attr('xlink:href');
      this.getTransformation = function() {
        return this.get('transform');
      };
      this.beforeDrawing = function() {
      ...
      this.getPaint = function(bBox, gOpacity, isClip, isMask) {
        ...
        if (matrix = validateMatrix(matrix)) {
          ...
          for (var n = 0; n < nTotal; n++) {
            ...
            for (var i = 0; i < children.length; i++) {
              var child = children[inOrder ? i : children.length - 1 - i],
                  stopColor = child.get('stop-color');
              if (stopColor === 'none') {stopColor = [255, 255, 255, 0];}
              var stopOpacity = stopColor[3] * child.get('stop-opacity') * gOpacity;
              if (stopOpacity < 1) {
                if (isMask) {
          ...
          if (markerStart !== 'none' || markerMid !== 'none' || markerEnd !== 'none') {
            ...
            if (markerMid !== 'none') {
              for (var i = 1; i < markersPos.length - 1; i++) {
                var marker = new SvgElemMarker(markerMid, null);
                marker.drawMarker(false, isMask, markersPos[i], this.get('stroke-width'));
              }
            }
            if (markerEnd !== 'none') {
              var marker = new SvgElemMarker(markerEnd, null);
    ...
    var SvgElemText = function(obj, inherits) {
      ...
      (function (textParentElem) {
        ...
        function recursive(currentElem, parentElem) {
          ...
          var fontOptions = {fauxItalic: false, fauxBold: false},
  ...
  //  console.log('typeof getComputed',(typeof getComputedStyle === 'function'));
    ...
    if (typeof fontCallback !== 'function') {
      fontCallback = function(family, bold, italic, fontOptions) {
        console.log("process font family bold italic fontOptions", family, !!doc._registeredFonts[ family ])
